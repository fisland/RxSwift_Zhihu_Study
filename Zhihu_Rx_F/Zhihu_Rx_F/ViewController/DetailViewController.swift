...
class DetailViewController: UIViewController {
    ...
    var statusBackView = UIView().then {
        ...
        $0.frame = CGRect.init(x: 0, y: 0, width: KScreenW, height: 20)
        $0.isHidden = true
    }
    ...
    override func viewDidLoad() {
        ...
        previousWeb = DetailWebView.init(frame: CGRect.init(x: 0, y: -KScreenH, width: KScreenW, height: KScreenH))
...
extension DetailViewController : UIScrollViewDelegate{
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        webview.maskImg.frame = CGRect.init(x: 0, y: webview.img.frame.size.height - 100, width: KScreenW, height: 100)
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        if scrollView.contentOffset.y <= -60 {
            if previousId > 0 {
                previousWeb.frame = CGRect.init(x: 0, y: -KScreenH, width: KScreenW, height: KScreenH)
                UIView.animate(withDuration: 0.3, animations: {
                    self.webview.transform = CGAffineTransform.init(translationX: 0, y: KScreenH)
                    self.previousWeb.transform = CGAffineTransform.init(translationX: 0, y: KScreenH)
                }, completion: { (state) in
                    if state { self.changeWebview(self.previousId) }
                })
            }
        }
        if scrollView.contentOffset.y - 50 + KScreenH >= scrollView.contentSize.height {
            if nextId > 0 {
                previousWeb.frame = CGRect.init(x: 0, y: KScreenH, width: KScreenW, height: KScreenH)
                UIView.animate(withDuration: 0.3, animations: {
                    self.previousWeb.transform = CGAffineTransform.init(translationX: 0, y: -KScreenH)
...
extension DetailViewController{
    ...
    func loadData() {
        provider.request(.getNewsDesc(id))
        .mapModel(NewDetailModel.self)
        .subscribe(onNext:{ model in
            ...
            if (model.title?.characters.count)! > 16 {
                self.webview.titleLab.frame = CGRect.init(x: 15, y: 120, width: KScreenW - 30, height: 55)
            }
...
extension DetailViewController : UIWebViewDelegate{
    ...
    func webViewDidFinishLoad(_ webView: UIWebView) {
        ...
        webview.nextLab.frame =  CGRect.init(x: 15, y: self.webview.scrollView.contentSize.height + 10, width: KScreenW - 30, height: 20)
    }
}
